...
module.exports={
    ...
    listUsers:(param,option)=>{
        if(option.task=='all'){
            ...
            return MainModel.find({}).select('_id email username name password');
        }
        if(option.task=='username'){
            return MainModel.findOne({username:param.username}).select('_id email name username password');
        }
        if(option.task=='email'){
            return MainModel.findOne({email: param.email }).select('_id email name username email password');
        }
        if(option.task=='usernameAndEmail'){
            return MainModel.findOne({email: param.email,username:param}).select('_id email name username email password');
        }
        if(option.task=='image'){
            return MainModel.findOne({email: param.email}).select('_id email name username email password image');
        }
